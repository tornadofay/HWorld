...
namespace HWorld.Example
{
    internal sealed class MainForm : Form
    {
        ...
        private Control BuildSidebar()
        {
            ...
            var source = MakeLabel("Create worlds, edit vector objects, then play inside them.", 8.7f, FontStyle.Regular, Color.FromArgb(146, 158, 171));
            ...
            var random = MakeButton("Generate");
            random.Width = 110;
            random.Height = 34;
            ...
            foreach (var shape in Enum.GetValues(typeof(WorldShapeKind)))
                _toolBox.Items.Add(shape);
            ...
            panel.Controls.Add(MakeSectionLabel("SELECTED OBJECT"));
            _propertiesPanel = BuildPropertiesPanel();
            panel.Controls.Add(_propertiesPanel);
            ...
            _hintValue.Dock = DockStyle.Top;
            _hintValue.Height = 74;
        ...
        private Panel BuildPropertiesPanel()
        {
            ...
            _nameBox = MakePropertyTextBox();
            AddProperty(panel, "Name", _nameBox);
            _kindBox = MakePropertyTextBox();
            AddProperty(panel, "Kind", _kindBox);
            ...
            foreach (var shape in Enum.GetValues(typeof(WorldShapeKind)))
                _shapeBox.Items.Add(shape);
            AddProperty(panel, "Shape", _shapeBox);

            _widthBox = MakeNumber(0.5m, 1000m, 0.5m);
            AddProperty(panel, "Width", _widthBox);
            _heightBox = MakeNumber(0.5m, 1000m, 0.5m);
            AddProperty(panel, "Height", _heightBox);
            _rotationBox = MakeNumber(-360m, 360m, 1m);
            AddProperty(panel, "Rotation", _rotationBox);

            _propertySolidBox = new CheckBox
            {
                Text = "Solid / collidable",
                Dock = DockStyle.Fill,
                ForeColor = Color.FromArgb(205, 213, 222),
            ...
            AddProperty(panel, "", _propertySolidBox);

            var apply = MakeButton("Apply changes");
            apply.Height = 34;
            apply.Click += delegate { ApplySelectedProperties(); };
            panel.Controls.Add(apply);

            var del = MakeButton("Delete selected");
            del.Height = 34;
        ...
        private TextBox MakePropertyTextBox()
        {
            return new TextBox
            {
                Dock = DockStyle.Fill,
        ...
        private NumericUpDown MakeNumber(decimal min, decimal max, decimal increment)
        {
        ...
        private void AddProperty(Control parent, string caption, Control value)
        {
            var row = new Panel { Dock = DockStyle.Top, Height = 30 };
            var label = MakeLabel(caption, 7.5f, FontStyle.Regular, Color.FromArgb(126, 139, 153));
            label.Dock = DockStyle.Left;
            label.Width = 70;
            row.Controls.Add(label);
            value.Location = new Point(74, 2);
            value.Width = 190;
        ...
        private void ClearPropertyControls()
        {
            ...
            _nameBox.Clear();
            _kindBox.Clear();
            _shapeBox.SelectedItem = null;
            _widthBox.Value = 1;
            _heightBox.Value = 1;
            _rotationBox.Value = 0;
            _propertySolidBox.Checked = false;
        ...
        private void LoadSelectedProperties()
        {
            ...
            _widthBox.Value = ClampDecimal((decimal)item.Width, _widthBox.Minimum, _widthBox.Maximum);
            _heightBox.Value = ClampDecimal((decimal)item.Height, _heightBox.Minimum, _heightBox.Maximum);
            _rotationBox.Value = ClampDecimal((decimal)item.RotationDegrees, _rotationBox.Minimum, _rotationBox.Maximum);
            _propertySolidBox.Checked = item.Solid;
            _updatingProperties = false;
        }
        ...
        {
            var l = MakeLabel(text, 8f, FontStyle.Bold, Color.FromArgb(104, 193, 228));
        ...
        private Label AddMetric(Control parent, string caption)
        {
            ...
            parent.Controls.Add(row);
            parent.Controls.SetChildIndex(row, 0);
            var l = MakeLabel(caption, 7.8f, FontStyle.Regular, Color.FromArgb(126, 139, 153));
            l.Dock = DockStyle.Top;
            l.Height = 15;
            row.Controls.Add(l);
            var v = MakeLabel("—", 9.5f, FontStyle.Bold, Color.FromArgb(225, 231, 237));
        ...
        private void SetMode(CanvasMode mode)
        {
            ...
            else
            {
                _running = false;
                _timer.Stop();
                _hintValue.Text = "OBSERVE  •  simulation paused";
            }
            UpdateStatus();
            _canvas.Invalidate();
        }
        ...
        private void UpdateStatus()
        {
            ...
            if (_modeValue != null) _modeValue.Text = _canvas.Mode.ToString();
            if (_timeValue != null) _timeValue.Text = _scenario.World.SimulationTime.ToString("0.00") + " s";

...
namespace HWorld.Example
{
    internal sealed class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            MinimumSize = new Size(1100, 700); StartPosition = FormStartPosition.CenterScreen;
            KeyPreview = true; BackColor = Color.FromArgb(11, 14, 18); ForeColor = Color.FromArgb(235, 239, 244);
            var root = new TableLayoutPanel { Dock = DockStyle.Fill, BackColor = BackColor, ColumnCount = 2, RowCount = 2, Padding = new Padding(10) };
            root.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 285f)); root.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 100f));
            root.RowStyles.Add(new RowStyle(SizeType.Absolute, 56f)); root.RowStyles.Add(new RowStyle(SizeType.Percent, 100f)); Controls.Add(root);
            ...
            header.PerformOnHelp += delegate { HMessage.ShowInformation(this, "Build mode: left-click to place the selected vector object. Right-click removes an object.\r\nPlay mode: WASD or arrow keys.\r\nMouse wheel zooms.", "HWorld controls"); };
            ...
            _timer = new Timer { Interval = 33 }; _timer.Tick += OnTick; KeyDown += OnKeyDown; KeyUp += OnKeyUp;
            FormClosing += delegate { _timer.Stop(); }; _canvas.WorldEdited += delegate { UpdateStatus(); };
            LoadHandBuiltWorld();
        }

        private Control BuildSidebar()
        {
            var panel = new Panel { Dock = DockStyle.Fill, BackColor = Color.FromArgb(20, 25, 32), Padding = new Padding(16) };
            ...
            var source = MakeLabel("Create worlds, paint vector objects, then play inside them.",8.7f,FontStyle.Regular,Color.FromArgb(146,158,171)); source.Dock=DockStyle.Top; source.Height=38; panel.Controls.Add(source);
            var hand=MakeButton("New hand-built world"); hand.Click+=delegate{LoadHandBuiltWorld();}; panel.Controls.Add(hand);
        ...
        private static Label MakeLabel(string text,float size,FontStyle style,Color color)=>new Label{Text=text,Font=new Font("Segoe UI",size,style),ForeColor=color,BackColor=Color.Transparent,AutoEllipsis=true};

        private void LoadHandBuiltWorld(){_timer.Stop();_running=false;_scenario=WorldScenarioFactory.CreateHandBuilt();ApplyScenario();SetMode(CanvasMode.Build);}
        private void GenerateSeededWorld(){int seed;if(!int.TryParse(_seedBox.Text.Trim(),out seed)){HMessage.ShowWarning(this,"Enter a valid integer seed.","Generate world");_seedBox.Focus();return;} _timer.Stop();_running=false;_scenario=WorldScenarioFactory.CreateSeeded(seed);ApplyScenario();SetMode(CanvasMode.Build);}
        private void ApplyScenario(){_canvas.World=_scenario.World;_canvas.Player=_scenario.Player;_canvas.Mode=CanvasMode.Build;_canvas.ResetView();_storyValue.Text=_scenario.Story;ApplyTool();UpdateStatus();}
        ...
        private void OnTick(object sender,EventArgs e){const double dt=1.0/30.0;if(!_running||_scenario==null)return;double x=0,y=0;if(_left)x-=1;if(_right)x+=1;if(_up)y-=1;if(_down)y+=1;if(Math.Abs(x)>0||Math.Abs(y)>0)_scenario.World.MoveActor(_scenario.Player.Id,x,y,dt);_scenario.World.Update(dt);UpdateStatus();_canvas.Invalidate();}
